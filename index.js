...
import { coinGeckoPrice } from "./src/commands/coinGeckoPrice/index.js";
import { binancePrice } from "./src/commands/binancePrice/index.js";
dotenv.config();
...
bot.command("binancePrice", async (ctx) => {
  await binancePrice(ctx);
});
bot.command("coinGeckoPrice", async (ctx) => {
  await coinGeckoPrice(ctx);
});
bot.launch();
